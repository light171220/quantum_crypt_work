...
def verify_s1_box():
    ...
    for i in range(16):
        ...
        s1_value = s1[row][col]
        s1_bits = [int(b) for b in format(s1_value, '02b')]
        ...
        circuit = s1_box_circuit(circuit, qreg)
        
        circuit.measure(qreg[0], creg[0])
        circuit.measure(qreg[1], creg[1])
        
        if input_bits == [1, 0, 1, 0]:
            circuit.draw(output='mpl', filename='s1_box_circuit_1010.png')
        ...
        classical_str = bits_to_str(s1_bits)
        quantum_str = bits_to_str(quantum_result)
        match = (s1_bits == quantum_result)
        ...
        if input_bits == [1, 0, 1, 0]:
            plot_histogram(counts, title="S1 Box Results for input 1010").savefig('s1_box_histogram_1010.png')
    
    print("S1 Box Test Results")
    print("-" * 40)
...
if __name__ == "__main__":
    success = verify_s1_box()
    print(f"S1 Box Verification: {'PASS' if success else 'FAIL'}")