...
public:
    ...
    uint8_t execute_circuit()
    {
        ...
        for (const auto &gate_info : circuit_config)
        {
            ...
            {
                if (gate_info.q1 != gate_info.q3 && gate_info.q2 != gate_info.q3)
                {
...
CircuitConfig simplify_circuit(const CircuitConfig &circuit)
{
    ...
    for (size_t i = 0; i < simplified.size(); i++)
    {
        if (simplified[i].gate_type == "ccx" &&
            (simplified[i].q1 == simplified[i].q3 || simplified[i].q2 == simplified[i].q3))
        {
        ...
        if (simplified[i].gate_type == "swap" && simplified[i].q1 == simplified[i].q2)
        {
            simplified.erase(simplified.begin() + i);
            i--;
            continue;
    ...
    {
        if (simplified[i].gate_type == "x" && i + 1 < simplified.size() &&
            simplified[i + 1].gate_type == "x" && simplified[i].q1 == simplified[i + 1].q1)
        {
    ...
    for (size_t i = 0; i < simplified.size() - 1; i++)
    {
        if (simplified[i].gate_type == "cx" && i + 1 < simplified.size() &&
            simplified[i + 1].gate_type == "cx" && simplified[i].q1 == simplified[i + 1].q1 &&
    ...
    for (size_t i = 0; i < simplified.size() - 1; i++)
    {
        if (simplified[i].gate_type == "ccx" && i + 1 < simplified.size() &&
            simplified[i + 1].gate_type == "ccx" && simplified[i].q1 == simplified[i + 1].q1 &&
            simplified[i].q2 == simplified[i + 1].q2 && simplified[i].q3 == simplified[i + 1].q3)
        {
            simplified.erase(simplified.begin() + i, simplified.begin() + i + 2);
            i--;
        }
    ...
    for (size_t i = 0; i < simplified.size() - 1; i++)
    {
        if (simplified[i].gate_type == "swap" && i + 1 < simplified.size() &&
            ((simplified[i].q1 == simplified[i + 1].q1 && simplified[i].q2 == simplified[i + 1].q2) ||
             (simplified[i].q1 == simplified[i + 1].q2 && simplified[i].q2 == simplified[i + 1].q1)))
        {
            simplified.erase(simplified.begin() + i, simplified.begin() + i + 2);
            i--;
        }
...
void ensure_swap_at_end(CircuitConfig &circuit, int max_swap_depth = 3)
{
    ...
    for (const auto &gate : circuit)
    {
        ...
        {
            if ((gate.q1 < 4 || gate.q2 < 4) && swap_gates.size() < max_swap_depth && gate.q1 != gate.q2)
            {
        ...
        {
            if (!(gate.gate_type == "ccx" && (gate.q1 == gate.q3 || gate.q2 == gate.q3)))
            {
...
private:
    ...
    CircuitConfig generate_random_circuit()
    {
        ...
        circuit.push_back(GateInfo("cx", 5, 0));
    ...
    void mutate_circuit(CircuitConfig &circuit)
    {
        ...
        for (const auto &gate : circuit)
        {
            ...
            {
                if (gate.q1 != gate.q3 && gate.q2 != gate.q3)
                {
                    non_swap_gates.push_back(gate);
                }
            }
        ...
        double add_prob = 0.9;
        double remove_prob = 0.9;
        double modify_prob = 0.9;
    ...
    double evaluate_fitness(const CircuitConfig &circuit)
    {
        ...
        int row_col_correct = 0;
        int critical_cases_correct = 0;
        const std::vector<uint8_t> critical_inputs = {0x00, 0x01, 0x1F, 0x20, 0x21, 0x3F};
        double gate_quality_score = 0.0;

        // Track gate usage patterns
        std::map<std::string, int> gate_counts;
        std::set<int> active_outputs;
        ...
        for (const auto &gate : circuit)
        {
            ...
            {
                active_outputs.insert(gate.q2);
            }
            else if (gate.gate_type == "ccx" && gate.q3 < 4)
            {
                active_outputs.insert(gate.q3);
            }
        }
        ...
        for (const auto &test_vec : test_vectors)
        {
            uint8_t input = test_vec.first;
            uint8_t classical = test_vec.second;
            uint8_t row = ((input & 0x20) >> 4) | (input & 0x01);
            uint8_t col = (input & 0x1E) >> 1;

            SBoxQuantumCircuit qc(input, circuit);
            uint8_t quantum = qc.execute_circuit();
            ...
            if (quantum == classical)
            {
                ...
                // Bonus for critical cases
                if (std::find(critical_inputs.begin(), critical_inputs.end(), input) != critical_inputs.end())
                {
                    critical_cases_correct++;
                }
            }
            // Partial credit for row/column correctness
            else
            {
                bool row_match = ((quantum & 0x01) == (classical & 0x01));
                bool col_match = ((quantum & 0x0E) == (classical & 0x0E));
        ...
        double perfect_match_rate = perfect_matches / 64.0;
        double row_col_accuracy = row_col_correct / 64.0;
        double critical_score = critical_cases_correct / critical_inputs.size();

        // Evaluate gate quality
        double output_coverage = active_outputs.size() / 4.0;
        double ccx_ratio = gate_counts["ccx"] / static_cast<double>(circuit.size());
        double useful_gate_score = 1.0 - (ccx_ratio * 0.3); // Penalize excessive CCX

        // Size penalty (non-linear)
        double size_penalty = exp(-0.01 * circuit.size());

        // Composite fitness
        return (0.7 * perfect_match_rate) +
               (0.3 * row_col_accuracy) +
               (0.1 * critical_score) +
               (0.05 * output_coverage) +
               (0.05 * useful_gate_score) *
                   size_penalty;
    }
    ...
    void repair_and_optimize_circuit(CircuitConfig &circuit)
    {
        ...
        for (const auto &gate : circuit)
        {
            if (gate.gate_type == "swap" && gate.q1 != gate.q2)
            {
                swap_gates.push_back(gate);
            }
            else if (gate.gate_type == "ccx" && gate.q1 != gate.q3 && gate.q2 != gate.q3)
            {
                non_swap_gates.push_back(gate);
            }
            else if (gate.gate_type != "ccx")
...
public:
    GeneticAlgorithm(int pop_size = 150, double mut_rate = 0.3, int max_g = 300, int threads = 8)
    ...
    void reset_population_partially()
    {
        ...
        std::vector<CircuitConfig> new_population;
        int keep_count = population_size * 0.9;
...
int main()
{
    ...
    GeneticAlgorithm genetic_algo(400, 0.15, 300, 32);
    auto [best_circuit, best_fitness] = genetic_algo.evolve(50000);
